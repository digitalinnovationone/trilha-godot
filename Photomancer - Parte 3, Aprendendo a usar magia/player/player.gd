...
@export_category("Cajado Temporário")
var wand_cooldown: float = 0.0
@export var wand_interval: float = 0.5
...
func _shoot() -> void:
	...
	wand_cooldown = wand_interval
	
	# Animar player
	animation_state_machine.start("Spellcast")
	
	# Criar projétil
	var projectile: RigidBody3D = magic_prefab.instantiate()
	add_sibling(projectile)
	projectile.global_position = projectile_spawn_marker.global_position
	projectile.linear_velocity = camera.global_transform.basis * Vector3(0, 0, -projectile_speed)
